...
export class ProjectDetailComponent implements OnInit {
  ...
  loadProject(id: number): void {
    ...
    setTimeout(() => {
      this.project = {
        ...
        description:
          'Complete redesign of the company website with modern UI/UX principles and responsive design for all devices.',
        status: 'active',
        priority: 'high',
        progress: 75,
  ...
  updateProgress(): void {
    if (this.project) {
      const completedTasks = this.project.tasks.filter(
        (t) => t.completed
      ).length;
      this.project.progress = Math.round(
        (completedTasks / this.project.tasks.length) * 100
      );
    }
  }
